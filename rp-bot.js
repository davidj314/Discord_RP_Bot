...
Client.on('message',  async message => {
    ...
    if (message.content.substring(0,3) === 'rp!') { 
		...
		get_char_id(guild_id, author_id, name, (cid)=>{get_card_info(guild_id, author_id, cid, (row)=>{show_card(row.url, row.upval, row.downval, row.leftval, row.rightval) ;}, (msg)=>{channel.send(msg)} )}, (msg)=>{channel.send(msg)} ) ;	
