...
function get_user_cards(server_id, owner_id, callback, bad){
    var select_query = "SELECT Card_Inv.cid, Names.name, Cards.upval, Cards.leftval, Cards.rightval, Cards.downval  FROM Cards INNER JOIN Names ON Cards.char_id=Names.id INNER JOIN Card_Inv ON Cards.id = Card_Inv.cid WHERE Card_Inv.server_id = $1 AND Card_Inv.owner_id = $2";
...
function get_user_made_cards(server_id, owner_id, callback, bad){
    var select_query = "SELECT Card_Inv.cid, Names.name, Cards.upval, Cards.leftval, Cards.rightval, Cards.downval, Cards.xp  FROM Cards INNER JOIN Names ON Cards.char_id=Names.id INNER JOIN Card_Inv ON Cards.id = Card_Inv.cid WHERE Card_Inv.server_id = $1 AND Cards.owner_id = $2";
    var query_values = [server_id, owner_id];
    var pool = new PG.Pool({ connectionString: process.env.DATABASE_URL, SSL: true});
    pool.query(select_query, query_values, (err, result) => {
        console.log(result);
...
Client.on('message',  async message => {
    ...
    if (message.content.substring(0,3) === 'rp!') { 
		...
		get_user_made_cards(guild_id, author_id, (rows)=>{
			var output = "CID   XP         Name                              Up               Left               Right          Down           Total        XP\n";
			var allcards = []
